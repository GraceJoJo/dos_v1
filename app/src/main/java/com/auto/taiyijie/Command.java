...
public class Command {
    ...
    //获取出货指令
    public byte[] commandGood(){
        byte cmd = 5;
        byte[] data = new byte[1];
        data[0] = this.channel;
        return getCommand(cmd,data);
    }

    //获取POLL查询指令
    public byte[] commandPoll(){
        byte cmd = 3;
        return getCommand(cmd);
    }

    //获取操作完成指令
    public byte[] commandEnd(){
        byte cmd = 6;
        return getCommand(cmd);
    }

    //组合要发送的指令，并对其添加CRC校验 【含数据】
    public byte[] getCommand(byte cmd,byte[] data){
        int len = (data == null)?4:4+data.length;
        byte[] buff = new byte[len-2];
        byte[] buff_ok = new byte[len];
        buff[0] = this.machine;
        buff[1] = cmd;
        if (data != null)
        {
        ...
        int crc =  CRC16_Check(buff);
        for(int i=0;i<len-2;i++){
            buff_ok[i] = buff[i];
        }
        buff_ok[len-2] = (byte)(crc & 0xff);
        buff_ok[len-1] = (byte)(crc >> 8);
        return buff_ok;
    }

    //组合要发送的指令，并对其添加CRC校验 【不含数组】
    public byte[] getCommand(byte cmd){
        int len = 4;
        byte[] buff = new byte[len-2];
        byte[] buff_ok = new byte[len];
        buff[0] = this.machine;
    ...
    public static int HexInt(byte b){
        ...
        coin_num = Integer.parseInt(hex,16);
        return  coin_num;
    }
